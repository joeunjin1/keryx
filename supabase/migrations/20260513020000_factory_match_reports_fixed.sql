...
-- 수정: profiles → user_profiles, factories 참조 제거
-- ============================================================
...
CREATE TABLE IF NOT EXISTS factory_match_report_items (
  ...
  -- 공장 기본 정보 (factories 테이블 없으므로 직접 입력)
...
-- 관리자/MD: 전체 접근 (user_profiles.kind 사용)
CREATE POLICY "admin_md_full_access_factory_match_reports"
  ...
  USING (
    EXISTS (
      ...
      AND user_profiles.kind::text IN ('admin', 'md', 'staff', 'operator')
    )
  );
